...
export class ComputeStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: ComputeStackProps) {
    ...
    const apiTransformationLambdaConstruct = new BasicDockerLambda(
      this,
      'ApiTransformationLambda',
      {
        ...
        cmd: [
          'aws_src_sample.lambdas.apig_post_lambda.api_post_lambda_handler',
        ], //
        environment: {
          OUTPUT_BUCKET_NAME: props.outputBucket.bucketName,
          FILE_TYPE_COUNTER_TABLE_NAME:
            props.transformationCounterTable.tableName,
        },
        memorySize: 512,
      }
    );
    ...
    props.transformationCounterTable.grantReadWriteData(
      this.apiTransformationLambda
    );

    const userProgressLambdaConstruct = new BasicDockerLambda(
      this,
      'UserProgressLambda',
      {
        ...
        imageTag: props.imageTag,
        cmd: [
          'aws_src_sample.lambdas.user_progress_lambda.user_progress_lambda_handler',
        ],
        environment: {
    ...
    const learningEntriesLambdaConstruct = new BasicDockerLambda(
      this,
      'LearningEntriesLambda',
      {
        functionNameSuffix: 'LearningEntries',
        description:
          'Handles API requests that GET/SET of learning entries (journal)',
        dockerRepository: props.dockerRepository,
        imageTag: props.imageTag,
        cmd: [
          'aws_src_sample.lambdas.learning_entries_lambda.learning_entries_lambda_handler',
        ],
        environment: {
